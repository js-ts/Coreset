...
double Squared_L2_Distance(vector<float> &first, vector<float> &second) {
        double distance = 0.0;
...
long double kmeans_obj(vector<vector<float> > &points, vector<vector<float> > &centers){
        long double cost = 0.0;
	int k = centers.size();
        
	for (size_t i = 0; i < points.size(); i++){
                double cur_min_dis = FLOAT_MAX_VALUE;
                for (int j = 0; j < k; j++){
                    double cur_dis = Squared_L2_Distance(points[i], centers[j]);
...
// compute the errors based on the centers generated from coreset and original data
void evaluation_2(string coreset_path, string coreset_centers_path, string coreset_labels_path, string coreset_weights_path, string data_path, string data_centers_path, string data_labels_path, string data_weights_path=""){
        ...
        long double coreset_inertia = kmeans_obj(data, coreset_centers);
        long double original_data_inertia = kmeans_inertia(data, data_centers, data_labels, data_weights);

        float relative_error = (coreset_inertia - original_data_inertia) / original_data_inertia;
...
int main(int argc, char **argv){
	...
	evaluation_2(coreset_path, coreset_centers_path, coreset_labels_path, coreset_weights_path, data_path, data_centers_path, data_labels_path);	
	return 0;
}
